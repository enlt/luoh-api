...
const getMimeType = (extension: string): string => {
    ...
    return mimeTypes[extension.toLowerCase()] || 'application/octet-stream'; // 如果找不到对应类型，返回默认类型
};
...
// 处理错误响应
const handleError = (res: NextApiResponse, message: string, startTime: number): void => {
    const responseTime = ((Date.now() - startTime) / 1000).toFixed(8);
    res.status(400).json({ status: '400', error: message, responseTime: `${responseTime}s` });
};
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (returnType === 'json') {
            const responseTime = ((Date.now() - startTime) / 1000).toFixed(8);
            return res.status(200).json({ status: 200, url: imageUrl, responseTime: `${responseTime}s` }); // 返回图片 URL 和响应时间
        }
        ...
        return handleError(res, '参数错误：无效的返回类型', startTime); // 返回类型无效时返回错误
    } catch (error) {
        return handleError(res, error instanceof Error ? error.message : '未知错误', startTime); // 捕获并处理错误
    }
}